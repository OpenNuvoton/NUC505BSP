...
extern int volatile gConfigIndex;

extern uint8_t volatile gSetConfig;
...
int  uac_config_parser(UAC_DEV_T *audev)
{
    ...
    result = USBH_GetDescriptor(udev, USB_DT_CONFIG, gConfigIndex, buffer, 8);
    if (result < 8)
    {
    ...
    result = USBH_GetDescriptor(udev, USB_DT_CONFIG, gConfigIndex, buffer, length);
    if (result != length)
    {
