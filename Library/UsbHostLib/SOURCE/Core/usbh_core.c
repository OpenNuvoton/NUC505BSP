...
int volatile gConfigIndex = 0;

static volatile uint32_t  g_devmap;

uint8_t volatile gSetConfig = 0;
uint8_t volatile giPhoneUAC = 0;
    ...
    for (cfgno = 0; cfgno < dev->descriptor.bNumConfigurations; cfgno++) {
        ...
        if (cfgno == 0) {
            ...
            if (result) {
				...
				usbh_mdelay(120); 	//Some device need delay after set configuration
...
/**
  * @brief    Set USB ipod device power.
  * @param[in] dev   The USB device.
  * @return  Success or not.
  * @retval   0  Success
  * @retval   Otherwise  Failed
  */
int USBH_SetiPodextra_power(USB_DEV_T *dev)
{
    int       ret;

    if ((ret = USBH_SendCtrlMsg(dev, usb_sndctrlpipe(dev, 0),
                                0x40, 0x40, 0x01f4,
                                0xfe0c, NULL, 0, HZ * 5)) < 0)
        return ret;
...
int  usbh_settle_new_device(USB_DEV_T *dev)
{
    ...
    err = usbh_settle_configuration(dev);
		...
		if(dev->descriptor.idVendor == 0x05AC && giPhoneUAC)
		{
				printf("iPhone UAC Device - idProduct 0x%04X\n", dev->descriptor.idProduct);
			
				USBH_SetiPodextra_power(dev);
				dev->act_iface = 1;
				dev->iface_alternate = 1;
				USBH_SetInterface(dev, dev->act_iface, dev->iface_alternate);
		}
