...
#include "ff.h"
//
// FIXME to drop sample(s) to align 4-byte address for SD reading
// __DEMO_ALIGN4BYTES__=0 if wav data already 4-byte alignment
//
#define __DEMO_ALIGN4BYTES__ 1
...
void WAVPlayer(void)
{
    ...
    do
    {
        ...
        while (1)
        {
            ...
            {
#if __DEMO_ALIGN4BYTES__
                //
                // FIXME to drop sample(s) to align 4-byte address for SD reading
                //
                f_lseek(&wavFileObject, wavDatPos + (f_tell(&wavFileObject) % 4));    // Seek to wave data.
#else
                //
                // for wav data already 4-byte alignment
                //
